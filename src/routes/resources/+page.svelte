...
<main class="min-h-screen">
  <!-- Hero -->
  <section class="relative bg-gradient-to-b from-neutral-800 to-neutral-900 text-white pt-20 pb-16 md:pt-24 md:pb-20 overflow-hidden">
  ...
  <section class="py-14 md:py-20 bg-[#f5f7fa]">
    <div class="container mx-auto px-4">
      {#if resources.length > 0}
        <ul class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 list-none p-0 m-0 w-full items-stretch">
          {#each resources as resource}
            <li class="flex h-full">
              <a
                href={withInternalUtm(`/${resource.url}`, 'resources')}
                class="group flex flex-col h-full w-full bg-white rounded-2xl p-6 shadow-sm border border-neutral-200/80 hover:border-[#d32f2f]/30 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#d32f2f]/50 focus:ring-offset-2"
              >
